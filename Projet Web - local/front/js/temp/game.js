class game{
    ...
    display(){
            ...
            for(let i = 0; i < 10; i++){
                for(let j = 0; j < 10; j++){
                    if (this.strategoTab.array[i][j] == 0 || this.strategoTab.array[i][j] == 1){
                      tabl.rows[i].cells[j].innerText = this.strategoTab.array[i][j];
                      ...
                      if (tabl.rows[i].cells[j].innerText == 1){
                        ...
                        tabl.rows[i].cells[j].innerText = this.strategoTab.array[i][j].name;
                        ...
                        tabl.rows[i].cells[j].addEventListener('click', () => {
                                if(pion){ // et que 
                                    if((i==pion[0]+1&&j==pion[1])||(i==pion[0]-1&&j==pion[1])||(i==pion[0]&&j==pion[1]+1)||(i==pion[0]&&j==pion[1]-1)){
                                        if(this.strategoTab.array[i][j] == 0){
                                            console.log("2e clique");
                                            this.strategoTab.array[i][j] = this.strategoTab.array[pion[0]][pion[1]];
                                            this.strategoTab.array[pion[0]][pion[1]] = 0;
                                            this.display();
                                            /*if(this.turn==1){this.turn=2;}
                                            else{this.turn=1;}*/
                                            pion = null;
                                        }
                                        else if(this.strategoTab.array[i][j] == 1){ // || this.strategoTab.array[i][j] == 'P'){
                                            alert("Vous ne pouvez pas aller ici");
                                            pion = null;
                                        }
                                        else{
                                            /*if(this.strategoTab.array[i][j].team==this.strategoTab.array[pion[0]][pion[1]].team){
                                    ...
                                    /*else if(this.strategoTab.array[i][j] == pion){
                                    ...
                                    */
                                    ...
                                    /*else if(this.strategoTab.array[i][j].team != this.turn){
    ...
    take_pion(i,j,x,y){
        if(this.strategoTab.array[i][j].power> this.strategoTab.array[x][y].power){
            if(this.strategoTab.array[i][j].name=="espion"&&this.strategoTab.array[x][y].name=="marechal"){
                this.strategoTab.array[x][y]=this.strategoTab.array[i][j];
                this.strategoTab.array[i][j]=0;
                this.display();
                return this.strategoTab.array;
            }
            else if(this.strategoTab.array[i][j].name=="marechal"&&this.strategoTab.array[x][y].name=="espion"){
                this.strategoTab.array[x][y]=this.strategoTab.array[i][j];
                this.strategoTab.array[i][j]=0;
                this.display();
                return this.strategoTab.array;
            ...
            /*
            else if(this.strategoTab.array[i][j].name=="demineur"&&this.strategoTab.array[x][y].name=="bombe"){
                this.strategoTab.array[x][y]=this.strategoTab.array[i][j];
                this.strategoTab.array[i][j]=0;
                this.display();
                return this.strategoTab.array;
            }
            else if(this.strategoTab.array[i][j].name=="bombe"||this.strategoTab.array[x][y].name=="bombe"){
                this.strategoTab.array[x][y]=this.strategoTab.array[i][j]=0;
                this.display();
                return this.strategoTab.array;
            }*/
            //else if(this.strategoTab.array[x][y].name=="drapeau"){this.endgame(this.strategoTab.array[i][j].team);}
            else{
                console.log(this.strategoTab.array[i][j].name+" a pris " +this.strategoTab.array[x][y].name);
                this.strategoTab.array[x][y]=this.strategoTab.array[i][j];
                this.strategoTab.array[i][j]=0;
                this.display();
                return this.strategoTab.array;
        ...
        else if(this.strategoTab.array[i][j].power== this.strategoTab.array[x][y].power){
            console.log(this.strategoTab.array[i][j]+" a pris " +this.strategoTab.array[x][y]);
            this.strategoTab.array[x][y]=this.strategoTab.array[i][j]=0;
            this.display();
            return this.strategoTab.array;
        }
        else{
            //if(this.strategoTab.array[i][j].name=="drapeau"){this.endgame(this.strategoTab.array[x][y].team);}
            //else{
                if(this.strategoTab.array[i][j].name=="demineur"&&this.strategoTab.array[x][y].name=="bombe"){
                    this.strategoTab.array[x][y]=this.strategoTab.array[i][j];
                    this.strategoTab.array[i][j]=0;
                    this.display();
                    return this.strategoTab.array;
                }
                console.log(this.strategoTab.array[x][y].name+" a pris " +this.strategoTab.array[i][j].name);
                this.strategoTab.array[i][j]=0;
                this.strategoTab.array[x][y]=this.strategoTab.array[x][y];
                this.display();
                return this.strategoTab.array;
            //}
